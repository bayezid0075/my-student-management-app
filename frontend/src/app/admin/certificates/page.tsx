  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-4xl font-bold text-gray-800 retro-font mb-2">Certificates</h1>
          ...
          <tbody>
            {certificates.length === 0 ? (
              ...
              certificates.map((cert) => (
                <tr key={cert.id}>
                  <td className="font-semibold">{cert.certificate_id}</td>
                  <td>{cert.student_details?.name || `Student ${cert.student}`}</td>
                  <td>{cert.course_details?.name || `Course ${cert.course}`}</td>
                  <td>{cert.batch_details?.name || '-'}</td>
                  <td>{new Date(cert.completion_date).toLocaleDateString()}</td>
                  <td>{new Date(cert.issued_at).toLocaleDateString()}</td>
                  <td>
                    <button
                      onClick={() => handleDownload(cert.id, cert.certificate_id)}
                      className="px-3 py-1 bg-retro-blue text-white rounded hover:opacity-90"
                    >
                      Download PDF
                    </button>
                  </td>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="retro-card max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <h2 className="text-2xl font-bold text-retro-blue mb-6">
